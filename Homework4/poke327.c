...
void init_trainers(uint16_t num_trainers, map_t *m)
{
  int x,y;
  ...
  if(num_trainers == 1){
    ...
    //Must be no trainers generated :(
  }
}

void set_logical_next(trainer_t *t, map_t *m){
  if(t->type == char_rival){
    //Find lowest cost around using rival dist map
    int min = INT_MAX;
    if((t->pos[0] == t->next_pos[0]) && (t->pos[1] == t->next_pos[1])){
      if(world.rival_dist[t->pos[1] - 1][t->pos[0] - 1] < min){ //top left
        min = world.rival_dist[t->pos[1] - 1][t->pos[0] - 1];
        t->next_pos[0] = t->pos[0] - 1;
        t->next_pos[1] = t->pos[1] - 1;
      }
      if(world.rival_dist[t->pos[1] - 1][t->pos[0]] < min){ //top
        min = world.rival_dist[t->pos[1] - 1][t->pos[0]];
        t->next_pos[0] = t->pos[0];
        t->next_pos[1] = t->pos[1] - 1;
      }
      if(world.rival_dist[t->pos[1] - 1][t->pos[0] + 1] < min){ //top right
        min = world.rival_dist[t->pos[1] - 1][t->pos[0] + 1];
        t->next_pos[0] = t->pos[0] + 1;
      ...
      if(world.rival_dist[t->pos[1] + 1][t->pos[0] - 1] < min){ //bottom left
        min = world.rival_dist[t->pos[1] + 1][t->pos[0] - 1];
        t->next_pos[0] = t->pos[0] - 1;
        t->next_pos[1] = t->pos[1] + 1;
      }
      if(world.rival_dist[t->pos[1] + 1][t->pos[0]] < min){ //bottom
        min = world.rival_dist[t->pos[1] + 1][t->pos[0]];
        t->next_pos[0] = t->pos[0];
        t->next_pos[1] = t->pos[1] + 1;
      }
      if(world.rival_dist[t->pos[1] + 1][t->pos[0] + 1] < min){ //bottom right
        min = world.rival_dist[t->pos[1] + 1][t->pos[0] + 1];
        t->next_pos[0] = t->pos[0] + 1;
        t->next_pos[1] = t->pos[1] + 1;
      }
    ...
    int min = INT_MAX;
    if((t->pos[0] == t->next_pos[0]) && (t->pos[1] == t->next_pos[1])){
      if(world.hiker_dist[t->pos[1] - 1][t->pos[0] - 1] < min){ //top left
        min = world.hiker_dist[t->pos[1] - 1][t->pos[0] - 1];
        t->next_pos[0] = t->pos[0] - 1;
        t->next_pos[1] = t->pos[1] - 1;
      }
      if(world.hiker_dist[t->pos[1] - 1][t->pos[0]] < min){ //top
        min = world.hiker_dist[t->pos[1] - 1][t->pos[0]];
        t->next_pos[0] = t->pos[0];
        t->next_pos[1] = t->pos[1] - 1;
      }
      if(world.hiker_dist[t->pos[1] - 1][t->pos[0] + 1] < min){ //top right
        min = world.hiker_dist[t->pos[1] - 1][t->pos[0] + 1];
        t->next_pos[0] = t->pos[0] + 1;
        t->next_pos[1] = t->pos[1];
      }
      ...
      if(world.hiker_dist[t->pos[1] + 1][t->pos[0] - 1] < min){ //bottom left
        min = world.hiker_dist[t->pos[1] + 1][t->pos[0] - 1];
        t->next_pos[0] = t->pos[0] - 1;
        t->next_pos[1] = t->pos[1] + 1;
      }
      if(world.hiker_dist[t->pos[1] + 1][t->pos[0]] < min){ //bottom
        min = world.hiker_dist[t->pos[1] + 1][t->pos[0]];
        t->next_pos[0] = t->pos[0];
        t->next_pos[1] = t->pos[1] + 1;
      }
      if(world.hiker_dist[t->pos[1] + 1][t->pos[0] + 1] < min){ //bottom right
        min = world.hiker_dist[t->pos[1] + 1][t->pos[0] + 1];
        t->next_pos[0] = t->pos[0] + 1;
        t->next_pos[1] = t->pos[1] + 1;
      }
    ...
    for (x = 0; x < MAP_X; x++) {
      if (world.trainers[y][x].pos[0] == -1) {
        ...
        if(world.trainers[y][x].type == char_rival){
          printf("r");
        } else if(world.trainers[y][x].type == char_hiker){
          printf("h");
        } else if(world.trainers[y][x].type == char_other){
          printf("o");
        } else if(world.trainers[y][x].type == char_pacer){
          printf("p");
        } else if(world.trainers[y][x].type == char_wanderer){
...
int main(int argc, char *argv[])
{
  ...
  init_trainers(num_trainers, world.cur_map);
