...
typedef struct map {
  ...
  int hiker_map[MAP_Y][MAP_X];
  int rival_map[MAP_Y][MAP_X];
...
int get_weight(map_t *m, int x, int y, char t){
  if(t == 'h'){
    ...
    if(m->map[y][x] == ter_boulder){
      return INT_MAX;
    } else if(m->map[y][x] == ter_tree){
      return INT_MAX;
    } else if(m->map[y][x] == ter_path){
      return 10;
    } else if(m->map[y][x] == ter_mart){
      return INT_MAX;
    } else if(m->map[y][x] == ter_center){
      return INT_MAX;
...
void dijkstra_hiker(map_t *m)
{
  ...
  {
	m->hiker_map[p->pos[dim_y]][p->pos[dim_x]] = p->cost;
...
void dijkstra_rival(map_t *m)
{
  ...
  for (y = 0; y < MAP_Y; y++) {
	...
	while ((p = heap_remove_min(&h)) && h.size > 0)
  {
	m->rival_map[p->pos[dim_y]][p->pos[dim_x]] = p->cost;

  // printf("Working on %d, %d\n", p->pos[dim_x], p->pos[dim_y]);
  // if(h.size % 20 == 0){
  //   printf("Size: %d\n", h.size);
  //   for (y = 0; y < MAP_Y; y++) {
  //     for (x = 0; x < MAP_X; x++) {
  ...
  //         printf("%3d", x);
  //       }
  //       else if (m->rival_map[y][x] == INT_MAX){
  //         printf("MAX");
  //       } else{
  //         int modded = m->rival_map[y][x] % 100;
  //         printf("%3d", modded);
  //       }
  //     }
  //     printf("\n");
...
static void print_hiker(map_t *m){
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      if(m->hiker_map[y][x] == INT_MAX){
        printf("   ");
      } else{
        int modded = (m->hiker_map[y][x] % 100);
        if(modded > 9){
          printf("%3d", modded);
        }
        else{
